export default function reducer (
  ...
  switch (action.type) {
    case 'FETCH_SF_LIST_PENDING': {
      return {...state, list: {fetching: true, fetched: false, code: 0, message: null}}
    }
    ...
    case 'FETCH_SF_USER_SORT_FULFILLED': {
      return {
        ...
        verify: {
          ...
          fetching: false,
          fetched: true,
          sort: action.payload.data.data,
          tabs: action.payload.data.data && action.payload.data.data.tabs.length > 1 ? action.payload.data.data.tabs : null,
          fields: action.payload.data.data && action.payload.data.data.tabs.length === 1 ? action.payload.data.data.tabs[0].field : null,
          code: action.payload.data.code,
          message: action.payload.data.message
        }
      }
