...
export default class Invite extends React.Component {
  ...
  componentDidMount () {
    ...
    get('http://credit.xianjincard.com/credit-invite/invite-four-act').then(data => data.data).then(data => {
      ...
      if (data.code === 1) {
        ...
        const inviteUrl = `http://test-h.xianjincard.com/activity/invite/share?tag=${tag || ''}&invite_code=${data.data.invite_code}`
        this.setState({
          loginState: data.code,
  ...
  invite () {
    ...
    if (this.buttonBefore()) {
      if (platform.isApp) {
        const url = `http://test-h.xianjincard.com/activity/invite/share?tag=${tag || ''}&invite_code=${this.state.inviteCode}`
        const imgUrl = 'http://test-h.xianjincard.com/assets/img/activity/invite/invite-08.png'
        partake({
          'share_title': '邀请返利4期',
          'share_body': '邀请返利4期主体',
          'share_url': url,
