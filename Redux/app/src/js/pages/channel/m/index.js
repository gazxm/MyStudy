...
import {Modal, RefreshControl, ListView, Carousel, Button} from 'antd-mobile'
import {openApp, forwardApp, get, post, redirect, login, resolveUrl, platform} from 'utils'
import Toast from 'components/Toast'
import Slider from 'pages/mobile/components/Slider'
...
export default class Loan extends React.Component {
  ...
  createLoan (data) {
    ...
    if (data.verify_loan_pass !== 1) {
      redirect.push(`/channel/m/certification${this.props.location.search}`)
      return
    }
  ...
  card (card, i) {
    const url = this.data.is_login === 1 ? '/mobile/order' : resolveUrl('http://h5.xianjincard.com/mobile/index.html#/login?redirect_url=' + encodeURIComponent(location.href))
    return (
      <div key={i} class={card.card_type === 2 ? 'wrap gold-card' : 'wrap'}>
        <h1>{card.card_title}</h1>
        <div class='credit' onClick={() => { location.href = url }}>
          ...
          <div class='info'><a href={resolveUrl('http://h.xianjincard.com/mobile/add-quota')}>提额攻略></a><a href={resolveUrl(`http://h.xianjincard.com/channel/m/certification${this.props.location.search}`)}>{card.card_verify_step}</a></div>
        </div>
      </div>
    )
  }
