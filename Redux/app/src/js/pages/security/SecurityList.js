...
export default class SecurityList extends React.Component {
  ...
  componentDidUpdate (prevProps, prevState) {
    ...
    if (fetched && items.length <= 0) location.href = '/security/certification' // redirect.replace('/security/certification')
  }
    ...
    return (
      <div class="transition-group fund-security-certification">
        ...
        <div class="custom-button">
          <Button className="btn" type="primary" onClick={() => { location.href = '/security/certification' }}>{items ? '继续添加' : '添加'}</Button>
        </div>
      </div>
    )
  }
