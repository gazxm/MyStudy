...
class SecurityCertification extends React.Component {
  ...
  createFields (fields) {
    ...
    return fields.map((field, i) => {
      ...
      return <InputItem labelNumber={field.parameter_name.length > 4 ? field.parameter_name.length : 0} type={field.parameter_type} key={i} {...getFieldProps(name, {rules: [{required: true, message: field.parameter_message}]})} clear placeholder={field.parameter_message} >{field.parameter_name}</InputItem>
    })
  }
