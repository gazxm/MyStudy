...
import {SegmentedControl, Modal, List, InputItem, Checkbox, Button, Picker, Toast, Icon, NoticeBar} from 'antd-mobile';
...
class Bank extends Component{
    ...
    submit(){
        ...
        let {login, form} = this.state;
        let field = [];
        let {entry_id, inputs} = login[form];
        (inputs && inputs.length) && inputs.map((input, index) => {
            field.push(`${input.name}@${form}@`);
        });
        validateFields([...field, 'agree'], (errors, values) => {
            if(errors){
                ...
                for(let i in data){
                    params[i.replace(/\@[0-9]+\@/, '')] = data[i];
                }
                if(lock){
                    this.setState({lock: 0, loading: 1});
                    request.post('credit-card/online-banking-login', params).then(response => {
    ...
    formChange(value){
        ...
        let values = login.map(data => data.title);
    ...
    render(){
        ...
        let {form, visible, captcha, status, login, lock, loaded, loading, agree, buttonText} = this.state;
        // let {inputs} = items;
        let input = (parengIndex, data, index) => {
            ...
            return (
                ...
                placeholder={desc}
                {...props}
                {...getFieldProps(`${name}@${parengIndex}@`, {
                    rules: [{
                        required: true,
                        message: `${label}不能为空`
                    }, rule]
            ...
            <div className='import'>
                {(login && login.length) &&
                    <div className='bank'>
                        ...
                        <SegmentedControl selectedIndex={form} values={login.map(data => data.title)} onValueChange={this.formChange.bind(this)}/>
                        <List>
                            {login.map(({inputs}, index) => (
                                (inputs && inputs.length && (index == form)) && inputs.map(input.bind(this, index))
                            ))}
                            <AgreeItem
                            ...
                            {...getFieldProps('agree', {
                                ...
                                我已阅读并同意<a onClick={() => window.location = url('http://credit.xianjincard.com/credit-web/loan-grant-auth')}>《用户授权服务协议》</a>
                            </AgreeItem>
                        </List>
                        <Button
                        type='primary'
