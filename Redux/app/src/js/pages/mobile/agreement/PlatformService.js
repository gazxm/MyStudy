...
export default class PlatformService extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      data: {
  ...
  render () {
    ...
    return (
      <div className='agreement platform-service'>
        <h1>平台服务协议</h1>
        ...
        <h3>第一条 协议成立</h3>
        <p>乙方在甲方或甲方合作方平台上通过类似点击确认按钮后，本协议及附件《法律文书送达地址确认书》即时成立并生效。本协议以数据电文的形式存储于甲方服务器或第三方电子数据存证平台中，乙方对此予以确认并同意。</p>
        <h3>第二条 定义</h3>
        ...
        <p>甲方及甲方合作方为乙方提供居间服务（居间撮合、征信审核、信息验证、账户管理、客户端使用等）并收取综合服务费。</p>
        <p>1、媒介服务：甲方利用自有资源和平台优势，为乙方在甲方平台上发起的借款寻找合适出借人，促成双方签订《借款协议》，实现乙方获得出借人出借款项；甲方有权根据业务经营情况为乙方选择介绍合适出借人的合作方居间平台，乙方同意于借款起息日，委托并授权出借人将乙方应付的相关款项，代乙方使用出借资金预先缴纳，或不可撤销地授权甲方指定的第三方支付机构从乙方账户划扣。</p>
        <p>2、贷后服务：《借款协议》生效后，甲方对《借款协议》项下借款及乙方还款提供服务，履行提醒、推荐义务，乙方在归还借款的同时向甲方及其合作方支付服务费用。贷后管理服务内容包括：</p>
        ...
        <p>乙方未能在《借款协议》任何一个还款日18:00前足额支付还款金额至乙方还款账户的，视作乙方逾期还款，甲方即对乙方的借款进行逾期管理，逾期管理服务及相关内容包括：</p>
        <p>1、直接或间接向乙方发送通知进行款项追偿，包括但不限于以手机短信、电话、信函、电子邮件、网站通知或其他合法方式提醒或催告乙方履行《借款协议》项下的还款义务；</p>
        <p>2、逾期管理款项的范围包括但不限于本金、利息、综合服务费、逾期费用以及实现债权的合理费用等，逾期还款按照优先偿还逾期费用以及实现债权的合理费用，其次综合服务费，再次借款利息，最后本金。</p>
        <p>3、乙方基于本合同项下借款产生的还款义务及服务费用等，有任何应付而未付的款项时即视为逾期，乙方需要向甲方支付逾期费用。</p>
        <p className='no-indent'>三、提前结清</p>
        <p>1、乙方可以发起提前结清服务申请，服务是否可用以操作界面展示为准（受银行结算等影响，部分日期可能无法发起申请）。</p>
        <p>2、乙方选择提前结清的：仍应按《借款协议》偿还事先约定的到期全额本息（包括本金、利息、综合服务费等）。</p>
        <h3>第四条 费用</h3>
        <p>1、综合服务费：指乙方因甲方或甲方合作方所提供的媒介服务、贷后管理等服务向甲方或甲方支付的费用：甲方在借款起息日收取。（详见附表）</p>
        ...
        <p>3、在《借款协议》约定的借款到期日的18:00之前，偿还根据《借款协议》约定的还款金额，并委托甲方将借款本息支付给出借人；</p>
        ...
        <table>
          <tbody>
            <tr>
              ...
              <th colSpan='4' style={{width: '55%'}}>
                <table style={{marginTop: '0'}}>
                  <tbody>
                    <tr>
                      <th colSpan='4' style={{border: 'none'}}>综合服务费</th>
                    </tr>
                    <tr>
                      <td style={{borderLeft: 'none', borderBottom: 'none'}}>平台服务费</td>
                      <td style={{borderBottom: 'none'}}>信息认证费</td>
                      <td style={{borderBottom: 'none'}}>风控服务费</td>
                      <td style={{borderRight: 'none', borderBottom: 'none'}}>风险准备金</td>
                    </tr>
                  </tbody>
                </table>
              </th>
            ...
            <tr>
              <td>{data.plan_list.plan_index}</td>
              <td>{data.plan_list.plan_date}</td>
              <td>{data.plan_list.remain_money}</td>
              <td>{data.plan_list.unpay_principal}</td>
              <td>{data.plan_list.unpay_interest}</td>
              <td style={{width: '13.75%'}}>{data.plan_list.service_fee}</td>
              <td style={{width: '13.75%'}}>{data.plan_list.info_fee}</td>
              <td style={{width: '13.75%'}}>{data.plan_list.credit_fee}</td>
              <td style={{width: '13.75%'}}>{data.plan_list.risk_fee}</td>
            </tr>
          </tbody>
        </table>
