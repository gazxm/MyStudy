...
import { Modal, Icon, List, Button, Checkbox, Popup } from 'antd-mobile'
...
export default class Loan extends React.Component {
  ...
  onChange = (i) => {
    ...
    // console.log(params)
    redirect.replace(`/mobile/loan/${params.card_type}/${params.period}/${params.money}/${params.coupon_id}${this.props.location.search}`)
  ...
  onCancel () {
    ...
    const {params} = this.props
    // console.log(params)
    redirect.push(`/mobile/loan/${params.card_type}/${params.period}/${params.money}${this.props.location.search}`)
  }
  onAgree (e) {
  ...
  onConfirm () {
    Password.show((value) => {
      setTimeout(() => this.confirm(value))
    }, '请输入交易密码')
  }
  confirm (password) {
    let {params} = this.props
    let {channel, channelSource, source_order: sourceOrder} = this.props.location.query
    params.money = this.state.money
    params.pay_password = password
    params.order_other = 2 // 来源m版定单
    channel && (params.channel_id = channel)
    channelSource && (params.channelSource = channelSource)
    sourceOrder && (params.source_order_id = sourceOrder)
    Toast.loading('加载中...')
    post('http://credit.xianjincard.com/credit-loan/apply-loan', params).then((result) => {
      ...
      if (data.code === 0) {
        // 借款成功
        Password.remove()
        data.data.item.tick && sendEventSDK(data.data.item.tick)
        if (channel !== undefined) {
          alert('申请成功', '放款结果可通过各应用市场下载"现金白卡"APP或关注微信公众号(xjbk88)查看哦', [
            { text: '我知道了', onPress: () => { location.href = '/channel/m' } }
          ])
          return
        }
        // redirect.push('/mobile/loan')
        location.href = '/mobile/loan'
        return
      }
