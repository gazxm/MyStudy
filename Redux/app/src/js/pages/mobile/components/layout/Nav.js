...
export default class Nav extends React.Component {
  render () {
    ...
    let { findFlag } = this.props.location.query
    findFlag && window.sessionStorage.setItem('findFlag', findFlag)
    if (!findFlag) {
      findFlag = window.sessionStorage.getItem('findFlag')
    }
    ...
    return (
      <Flex class='nav'>
        ...
        {findFlag !== '1' ? <Flex.Item class={findClass}><Link class='find' to='/mobile/find'>发现</Link></Flex.Item> : null}
