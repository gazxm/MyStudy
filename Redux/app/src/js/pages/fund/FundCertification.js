...
class FundCertification extends React.Component {
  ...
  createFields (fields) {
    ...
    return fields.map((field, i) => {
      ...
      return <InputItem labelNumber={field.parameter_name.length > 4 ? field.parameter_name.length : 0} type={field.parameter_type} key={i} {...getFieldProps(field.parameter_code, {rules: [{required: true, message: field.parameter_message + ' '}]})} clear placeholder={field.parameter_message} >{field.parameter_name}</InputItem>
    })
  }
  ...
  pickerCity (citys) {
    ...
    if (!Array.isArray(citys)) {
      Toast.info('城市数据格式不是数组，稍后重试', 2)
      return
    }
    if (citys.length === 0) {
      Toast.info('城市数据数组为空，稍后重试', 2)
      return
    }
    return (
      <Picker extra="请选择" cols={2} data={citys} {...getFieldProps('city', {onChange: this.onChange, rules: [{required: true, message: '请选择城市'}]})} title="选择地区">
