...
export default class FundList extends React.Component {
  ...
  componentDidUpdate (prevProps, prevState) {
    ...
    if (fetched && items.length <= 0) location.href = '/fund/certification' // redirect.replace('/fund/certification')
  }
    ...
    return (
      <div class="transition-group fund-security-certification">
        ...
        <div class="custom-button">
          <Button className="btn" type="primary" onClick={() => { location.href = '/fund/certification' }}>{items ? '继续添加' : '添加'}</Button>
        </div>
      </div>
    )
  }
