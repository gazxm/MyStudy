...
export default class Steptwo extends React.Component {
  ...
  forward (url, status, name) {
    if (status) {
      statistics({
        type: '智能测评',
        tag: `${name}: 已认证`
      }).then(() => {
        location.href = resolveUrl(`http://h.xianjincard.com/activity/evaluation/result?mention=1`)
      })
      return
    }
    statistics({
  ...
  render () {
    ...
    const content = <div className='job'>
      ...
      <div className='content'>
        <ul>
          {data ? data.map((v, i) => <li key={i} onClick={() => this.forward(v.h5_link, v.status, v.name)} className={`${v.status ? 'complete' : ''} ${data.length === 1 ? 'single' : ''}`}>
