...
const data = [{
	...
	description: {
		...
		buttonText: '点击领取',
		link: 'http://h.xianjincard.com/activity/novice/xinshou'
	}
}, {
	name: '五单五折',
			...
			return (
				<div key={index} className={className}>
					...
					<div className='description'>
						...
						: ''}
						{description.buttonText ? <div className='button' onClick={() => {
							if(description.link){
								window.location.href = description.link
							}else{
								goHome()
							}
						}}>{description.buttonText}</div> : ''}
					</div>
				</div>
			);
		}
