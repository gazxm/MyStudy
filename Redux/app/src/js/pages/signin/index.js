...
export default class Signin extends Component{
    constructor(props){
        ...
        this.state = {
            ...
            events: [{
                name: '去抽奖',
                icon: require('../../../assets/img/signin/upload/1.png'),
                link: 'activity/eggs?t=' + Date.now()
            }, {
                name: '去兑换',
                icon: require('../../../assets/img/signin/upload/2.png'),
                link: 'integral-mall?t=' + Date.now()
            }],
            // 卡片
    ...
    middleware(response){
        ...
        hide();
        if([-1000, -1004, -1014].indexOf(code) >= 0){
            type = 'fail';
    ...
    // 查看明细
    detailed(){
        window.location.href = 'signin/detailed?t=' + Date.now();
    }
    // 订阅
    subscribe(){
        let {show, hide} = this.loading();
    ...
    render(){
        ...
        return (
            <div className={`wrapper wrapper-signin ${loaded ? 'loaded' : 0}`}>
                ...
                <Tasks loaded={loaded} data={tasks} middleware={this.middleware.bind(this)} receive={this.receive.bind(this)}/>
                <ul className='events'>
                    {
                        events.map((event, index) => (
                            <li key={index}>
                                <a className='icon' href={event.link} style={{backgroundImage: `url(${event.icon})`}}/>
