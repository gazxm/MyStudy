...
export default class Tasks extends Component{
    ...
    componentDidMount(){
        ...
        request('credit-gold/task-list').then(response => {
            let {gold_task_banner, task} = response.data;
            let gallery = (gold_task_banner && gold_task_banner.items);
            this.setState({
                gallery,
                task,
                loaded: 1
    ...
    render(){
        ...
        return (
            <div className={classnames({'wrapper wrapper-integral-tasks': true, loaded: loaded})}>
                {gallery ?
