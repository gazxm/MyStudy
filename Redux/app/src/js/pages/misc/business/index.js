...
export default class Welcome extends Component {
  ...
  submit () {
    ...
    if (!lock) {
      if (!channel_name.value) {
        Toast.offline('渠道商名称不能为空', 3, () => {
          channel_name.focus()
        })
        return
      } else if (!relation_name.value) {
        Toast.offline('姓名不能为空', 3, () => {
          relation_name.focus()
        })
        return
      } else if (!relation_phone.value) {
        Toast.offline('电话不能为空', 3, () => {
  ...
  render () {
    ...
    return (
      <div className={classnames({'wrapper wrapper-misc-business': true, loaded})}>
        <div className='form'>
          ...
          <div className='form-group'>
            <Picker className='areas' cols={2} data={data} value={value} extra='合作区域' format={this.format.bind(this)} onChange={this.areas.bind(this)} onPickerChange={this.areas.bind(this)}>
              <Item className={classnames({'am-area-item': true, active: !!value})} arrow='down' />
            </Picker>
          </div>
          <div className='form-group'>
